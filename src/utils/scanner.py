...
from src.utils.search_utils import (
    populate_whoosh_index,
)  # Importar la función de indexación
...
async def scan_directory(directory, model, file_extension, category=None):
    ...
    for root, _, files in os.walk(
        directory
    ):  # Usar os.walk para recorrer subdirectorios
        for filename in files:
            if filename.endswith(file_extension):
                ...
                if not category:
                    ...
                    category = relative_path.split(os.sep)[
                        0
                    ]  # Primer nivel del subdirectorio
