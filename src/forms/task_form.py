...
class TaskForm(FlaskForm):
    ...
    start_date = DateTimeLocalField(
        "Fecha de inicio", format="%Y-%m-%dT%H:%M", validators=[DataRequired()]
    )
    end_date = DateTimeLocalField(
        "Fecha de fin", format="%Y-%m-%dT%H:%M", validators=[DataRequired()]
    )
    type = SelectField(
        "Tipo",
        choices=[
    ...
    end_recur = DateTimeLocalField(
        "Fecha de fin de recurrencia", format="%Y-%m-%dT%H:%M", validators=[Optional()]
    )
