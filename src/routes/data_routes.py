...
class MyRenderer(mistune.HTMLRenderer):
        ...
        try:
            ...
            if src and src.startswith("obsidian:"):
                ...
                try:
                    ...
                    obsidian_filename = base64.b64decode(encoded_filename).decode(
                        "utf-8"
                    )
                    current_app.logger.info(
                        f"Detectada imagen de Obsidian: {obsidian_filename}"
                    )
                    filename = obsidian_filename
                except Exception as e:
                    current_app.logger.error(
                        f"Error decodificando nombre de archivo Obsidian: {e}"
                    )
                    filename = None
            ...
            # Si src contiene "alt " al principio (indicando que Mistune pasó el alt como src)
                ...
                if image_pattern:
                    ...
                    current_app.logger.info(
                        f"Nombre de archivo extraído del alt: {filename}"
                    )
                else:
                    # Si no se encuentra un patrón de imagen, intentar buscar por el nombre en el directorio
                    current_app.logger.warning(
                        f"No se encontró patrón de imagen en: {alt_text}"
                    )
                    filename = None
            # Construir la ruta de la imagen
            if not filename:
                current_app.logger.warning(
                    f"No se pudo extraer un nombre de archivo válido: {src}"
                )
                return f'<img src="{src}" alt="{alt}" title="{title}" />'
            ...
            if matched_filename:
                ...
                current_app.logger.warning(
                    f"No se encontró ninguna imagen que coincida con: {filename}"
                )
                return f'<img src="{src}" alt="{alt}" title="{title}" />'

        except Exception as e:
            current_app.logger.error(
                f"Error al renderizar la imagen: {src} - {e}", exc_info=True
            )
    ...
    def block_code(self, code, lang=None, info=None):
        ...
        try:
            ...
            current_app.logger.error(
                f"Error al resaltar el bloque de código (lang={lang}): {e}",
                exc_info=True,
            )
...
def view_image(base_dir, category, filename):
    ...
    try:
        ...
        for root, dirs, files in os.walk(img_dir):
            ...
            for file in files:
                if file.lower() == filename.lower() or filename.lower() in file.lower():
                    current_app.logger.info(
                        f"Imagen encontrada en subdirectorio: {os.path.relpath(root, img_dir)}/{file}"
                    )
        ...
        current_app.logger.error(
            f"Error al servir la imagen {filename}: {e}", exc_info=True
        )
...
def delete_objetive(objetive_id):
    ...
    try:
        ...
        current_app.logger.error(
            f"Error deleting objetive {objetive_id}: {e}", exc_info=True
        )
    return redirect(url_for("data.checklist"))
...
def view_theory(theory_id):
    ...
    try:
        ...
        current_app.logger.error(
            f"Error al renderizar el contenido: {e}", exc_info=True
        )
